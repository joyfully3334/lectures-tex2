...
\usepackage{comment}
\newcommand{\commenting}[1]{\begin{comment}#1\end{comment}}
...
positioning, fit, 3d, perspective, decorations.pathmorphing}
...
\newcommand{\stylespecific}[5][]{
  \ifstrempty{#1}{\def\tmpstyleoo{\mathstyle}}{\def\tmpstyleoo{#1}}
  \ifnum \tmpstyleoo<1     % Display style
    \ifstrempty{#2}{#3}{#2}
  \else
    \ifnum \tmpstyleoo<4   % Text style
      #3
    \else
      \ifnum \tmpstyleoo<6 % Script style
        \ifstrempty{#4}{#3}{#4}
      \else                % Script script style
        \ifstrempty{#5}{
          \ifstrempty{#4}{#3}{#4}
        }{#5}
      \fi
...
\newcounter{styll}
\newcommand{\tmpstyle}[1]{
  \setcounter{styll}{\mathstyle}
  #1
  \stylespecific[\thestyll]{\displaystyle}{\textstyle}
...
\newcommand{\dlm} [2]  {\lim\limits_{\substack{#1 \\ #2}}}
\newcommand{\sm}  [2]  {\tmpstyle{\displaystyle \sum_{#1}^{#2}}}
\newcommand{\smm} [2]  {\sum\limits_{#1}^{#2}}
\newcommand{\prd} [2]  {\tmpstyle{\displaystyle \prod_{#1}^{#2}}}
\newcommand{\prdm}[2]  {\prod\limits_{#1}^{#2}}

\newcommand{\theoremindex}[6]{
  \ifstrempty{#1} {
    \index{#4#2!\arabic{chapter}?\arabic{section}!\alph{#6}@#3 #5.}
  }{
    \index{#4#2!\arabic{chapter}?\arabic{section}!\alph{#6}@#3 #5. #1}
  }
}

% Add fallback for mathbb
\makeatletter
\let\@mathbb\mathbb
\makeatother
...
\renewcommand{\mathbb}[1]{
  \ifnum \mathstyle=-1 #1 \else \@mathbb{#1} \fi
}
\makeatother
...
\newcommand{\jexercise}   [1]{\begin{exercise}#1\end{exercise}}
\newcommand{\jexrc}       [1]{\jexercise{#1}}
\newcommand{\jexc}        [1]{\jexercise{#1}}
\newcommand{\jexr}        [1]{\jexercise{#1}}

\newcommand{\jproblem}    [1]{\begin{problem}#1\end{problem}}
...
\newcommand{\jnote}       [1]{\begin{note}#1\end{note}}
\newcommand{\jnt}         [1]{\jnote{#1}}

\newcommand{\jreminder}   [1]{\begin{reminder}#1\end{reminder}}
\newcommand{\jremind}     [1]{\jreminder{#1}}
\newcommand{\jrmnd}       [1]{\jreminder{#1}}
\newcommand{\jrmd}        [1]{\jreminder{#1}}

\newcommand{\jexample}    [1]{\begin{example}#1\end{example}}
...
\newcommand{\fa} [1]{\left\langle #1 \right\rangle} % Angles

\newcommand{\sfd} [1]{(       #1 )}       % Small Parentheses
\newcommand{\sfb} [1]{[       #1 ]}       % Small Brackets
\newcommand{\sfp} [1]{|       #1 |}       % Small Pipes
\newcommand{\sfpp}[1]{\|      #1 \|}      % Small double PiPes
\newcommand{\sfdp}[1]{\|      #1 \|}      % Small Double Pipes
\newcommand{\sfP} [1]{\|      #1 \|}      % Small double Pipes
\newcommand{\sfc} [1]{\{      #1 \}}      % Small Curly braces
\newcommand{\sfa} [1]{\langle #1 \rangle} % Small Angles
...
\newcommand{\vc}[1]{
  ...
  \stylespecific{}{
    \tk[baseline]{
      \def\vup{0.39ex}
      \def\ladd{0em}
      \IfEqCase{#1}{{l}{\def\ladd{.04}}{t}{\def\ladd{.04}}}

      \node[inner sep=0pt, anchor=base] (char) {$#1$};
      \draw[-{Triangle[length=.7mm, width=.6mm]}]
      ($(char.north west) + (0.03-\ladd,\vup)$) -- ($(char.north east)+(\ladd,\vup)$);
    }
  }{
    \tk[baseline]{
      \def\vup{0.27ex}
      ...
      \draw[-{Triangle[length=.6mm, width=.5mm]}]
      ($(char.north west) + (0.03-\ladd,\vup)$) -- ($(char.north east)+(\ladd,\vup)$);
    }
  }{
    \tk[baseline]{
      \def\vup{0.22ex}
      \def\ladd{0em}
      \IfEqCase{#1}{{l}{\def\ladd{.03}}{t}{\def\ladd{.03}}}

      \node[inner sep=0pt, anchor=base] (char) {$\scriptscriptstyle #1$};
      \draw[-{Triangle[length=.5mm, width=.4mm]}]
      ($(char.north west) + (0.03-\ladd,\vup)$) -- ($(char.north east)+(\ladd,\vup)$);
    }
  }
}
...
\newcommand{\inditn} [1]{\ \hfill
  \setcounter{indchap}{0}
  \setcounter{indsec}{0}
  \tk[overlay]{
    \tp{(-0.27, 0) node {\large #1}}
  }
}

\newcommand{\indito} [1]{\ \hfill \vspace{1ex}
  \IfSubStrBefore{#1}{\theindchap}{?}{
    \tk[overlay, gray]{\tdraw{(-2.7, 0)--(2.7, 0)}
      \tnode[fill=white]{
        {\footnotesize \S \StrBehind{#1}{?}[\tmpc]\tmpc\setcounter{indsec}{\tmpc}}
      }
    }
  }
  {
    \tk[overlay, gray]{\tdraw{(-4, 0)--(4, 0)}
      \tnode[fill=white]{
        {\small Глава \StrBefore{#1}{?}[\tmpp]\tmpp\setcounter{indchap}{\tmpp}}
      }
    }
    \StrBehind{#1}{?}[\tmpd]
    \setcounter{indsec}{\tmpd}
  }
  \setcounter{indnum}{0}
}
...
\newcommand{\inditt} [1]{
  ...
  \else
    ...
    \fi
  \fi
  \setcounter{indnum}{\tmp}
  \IfBeginWith{#1}{Т}{\def\tmps{theorem}}{
    \IfBeginWith{#1}{Л}{\def\tmps{lemma}}{
      \IfBeginWith{#1}{У}{\def\tmps{proposition}}{
        \IfBeginWith{#1}{О}{\def\tmps{definition}}{\def\tmps{cringe}}
      }
    }
  }
  \small \hyperref[\tmps\theindchap.\theindsec.\theindnum]{#1}
  \dotfill
  \pageref{\tmps\theindchap.\theindsec.\theindnum}
}
\newcommand{\inditx} [1]{\iffalse #1 \fi}
