...
\newcommand{\tmatrix}[2][]{
  \matrix[matrix of nodes, ampersand replacement=\&, #1]{\replace{#2}{&}{\&}};
}
\newcommand{\tm}     [2][]{\tmatrix[#1]{#2}}

%%% Свои векторы TODO: Сделать автоматический размер
\newcommand{\vc}[1]{
  \ifnum \mathstyle>6
    \tk{
      \def\vup{0.16ex}
      \node[inner sep=0pt] (char) {$\scriptscriptstyle #1$};
  ...
  \else
    \ifnum \mathstyle>4
      \tk{
        \def\vup{0.2ex}
        \node[inner sep=0pt] (char) {$\scriptstyle #1$};
