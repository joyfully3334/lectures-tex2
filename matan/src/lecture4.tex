...
\jnt{
  ...
  точке $\vc x_0$ следует дифференцируемость в точке $\vc x_0$, а также $f \pm
  g, f \cdot g, \frac{f}{g} (g(\vc x_0) \neq 0)$, и $d(f \pm g) = df \pm g,
  d(f \cdot g) = g \cdot df + f \cdot dg, d\fd{\frac{f}{g}} = \frac{g \cdot df -
  f \cdot dg}{g^2}$ в точке $\vc x_0$
}
...
= \dt f(\vc y_0) = (\grad f(\vc y_0), \dt y) + o(\fp{\dt y}) = \\
  ...
  o(\fp{\dt y}) = \\
  = \grad f(\vc y_0) \cdot
  \fd{\jarr{c}{(\grad g_1(\vc x_0), \dt x) \\ \vdots \\ \grad (\grad g_n(\vc x_0), \dt x)}} +
  \grad f(\vc y_0) \cdot
