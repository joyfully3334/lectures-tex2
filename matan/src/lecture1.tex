...
\jen{
  \item
    ...
    В этом случае $r = \frac{p}{q}, p \in \Z, q \in \N$, тогда введем следующую
    замену: $\frac{ax + b}{cx + d} = t^q$, и получим
    \[ (ax + b) - (cx + d)t^q = 0 \iff
    x = \frac{dt^q - b}{a - ct^q}\]
    \[ \fd{\frac{ax + b}{cx + d}}^q = t^p \]
  Подвид вида
    \[ \int R\fd{x, \fd{\frac{ax + b}{cx + d}}^r, \ldots,
    \fd{\frac{ax + b}{cx + d}}^{r_n}}dx \]
  приводится к рациональной функции аналогично, но в качестве $q$ берем
  $\NOK$ от $r_1, \ldots, r_n$.
  \item 
    \[ \int R\fd{x, \sqrt{ax^2 + bx + c}}dx,\quad a \neq 0 \]
    ...
    \jen{
      ...
      \item $c > 0$
        ...
        Снова возведем в квадрат и получим
      ...
      \item
        ...
        В данном случае
        \[ \sqrt{ax^2 + bx + c} = \pm \sqrt{\fp{a}} (x - x_1)t \]
    ...
    Продифференцируем:
    \[ \fd{x^{m - 1}y}' = (m - 1)x^{m - 2}y + \frac{x^{m - 1}\fd{2ax + b}}{2y} \]
    Тогда
