...
\jprf{
  ...
  Докажем, что $G_2$
  открыто в $G$. Пусть $\vc x_0 \in G_2 \subset G$ тогда
  $\exists \epsilon > 0: U_\epsilon(\vc x_0) \subset G$. Отсюда $U_\epsilon(\vc
  x_0) \subset G_2$, значит $G_2$ --- открыто.

  Теперь докажем, что $G_1$ открыто. От противного,
...
\tpic{Эээ}{
  \tdraw[-Stealth]{(-2.6, -2)--(2.8, -2)}
  \tdraw[-Stealth]{(-2, -2.6)--(-2, 2.2)}
  \tp[fill=black!10]{(0, 0) circle (1.4)}
  % \tp{(0, 0) node {$\vc x_0$}}
  \tp[fill]{(0, 0) circle (.03) node[below left] {$\vc x_0$}}
  \tp[fill]{($(0, 0)+(.5, .5)$) circle (.03) node[above right] {$\vc x_0 + \dt
  \vc x$}}
}
