...
\jdf[Предел]{
  ...
  \[ \mlim[] f(\vc x) = l \iff \forall \epsilon > 0 \then \exists \delta > 0 :
  \forall \vc x \in D, 0 < \rho^x(\vc x, \vc x_0) < \delta \hk \rho_y(f(\vc x),
l) < \epsilon \]

  по Гейне:

  \[ \mlim[] f(\vc x) = l \iff \forall \fc{\vc x_m} \subset D: \mlim[m \to
  \infty] \vc x_m = \vc x_0, \vc x_m \neq \vc x_0 \hk \mlim[m \to \infty] f(\vc
  x_m) = l \]

  Если $\vc x_0$ -- внутренняя точка D, то $\mlim[\vc x \to \vc x_0]$ называется
  ...
  $\mlim[\vc x \to \vc x_0] f(\vc x)$ называется пределом по направлению $\vc d$
}

\jlm{
  \[ \mlim[\vc x \to \vc x_0] f(\vc x) = \mlim[t \to 0+] f(\vc x_0 + t\vc d) \]
...
\jth{
  ...
  $\mlim[\vc x \to \vc x_0] f(\vc x)$, то существует предел по любому
  направлению $\mlim[\vc x \to \vc x_0]$. Обратное, вообще говоря не верно.
}
...
\jexm{
  ...
  \[ \mlim[(x, y) \to (0, 0)] f(x, y) = \mlim[t \to 0+] f(0 + t\alpha, 0 +
  t\beta) = \mlim[t \to 0+]\frac{t^3\alpha\beta^2}{t^2\alpha^2 + t^4\beta^4} = 0 \]
...
\jdf[Повторный предел]{
  \[ \mlim[x \to x_0] \fd{\mlim[y \to y_0] f(x, y)} \]
}
...
\knt{
  ...
  \jexm{
    \[ g(x, y) = \left\{\jalg{&\sqrt{x^2 + y^2} \sin \frac{1}{x} \sin \frac{1}{y}\\
    &0}\right. \]

    \[ \mlim[(x, y)\to(0, 0)] g(x, y) = 0 \text{, так как } \fp{g(x, y)} \le
    \fp{(x, y)} \]

    \[ \mlim[y \to 0]{g(x, y)} \text{ существует и равен 0} \iff x = 0
    \text{ или } x = \frac{1}{k}, k \in K \]
  }
}

Если $\vc x_0$ -- предельная точка больати определения $D(f)$, то $f$ непрерывна
в $\vc x_0$ тогда и только тогда, когда $\mlim[x \to \vc x_0] f(\vc x) = f(\vc
x_0)$
...
\th[Непрерывность сложной функции многих переменных]{
...
\jpf{
  ...
  \[ \fc{\vc x_k} \subset D(h): \mlim[k \to \infty] \vc x_k = \vc x_0 \hk
  \mlim[k \to \infty] g_j(\vc x_k) = g_k(\vc x_0) = y_0^{(j)} \implies
...
\jprf{
  ...
  По критерий компактности $\exists \fc{\vc x_{1, k}_j}, \mlim[j \to \infty] \vc
  x_{1, k}_j = \vc x_0 \in K$

  Тогда

  \[ \fp{\vc x_{2, k}_j - \vc x_0} \le \fp{\vc x_{2, k}_j - \vc x_{1, k}_j} +
  \fp{\vc x_{1, k}_j - \vc x_0} \implies
  \mlim[j \to \infty] \vc x_{2, k}_j = \vc x_0\]

  Так как $f$ непрерывна в $\vc x_0$, то

  \[ \mlim[j \to \infty] f(\vc x_{1, k}_j) = \mlim[j \to \infty] f(\vc x_{2,
  k}_j) = f(\vc x_0) \]
